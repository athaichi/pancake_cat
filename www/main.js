...
async function run() {
  ...
  const runtime = await fetchWithProgress(
    ...
    (loaded, total) => {
      if (!total) {
        const mb = (loaded * 0.000001).toFixed(2);
        fillText(context, `Initializing runtime... ${mb}mb`);
      } else {
  ...
  let turbofile = await fetchWithProgress("main.turbo", (loaded, total) => {
    if (!total) {
      const mb = (loaded * 0.000001).toFixed(2);
      fillText(context, `Loading game data... ${mb}mb`);
    } else {
      const percent = Math.round((loaded / total) * 100);
  ...
  if (contents.has_audio()) {
    fillText(context, "CLICK TO START GAME");
